...
> = (state) => {
  ...
  const onPaletteClick = (hex: string) => {
    if (colorInput) {
      colorInput.color = hex;
    }
  };

  const paletteColors = ["#ef4444", "#3b82f6", "#22c55e", "#eab308", "#a855f7", "#f97316"];
          ...
          <bim-context-menu>
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
              ...
              <div style="display: flex; gap: 0.25rem;">
                ${paletteColors.map(c => BUI.html`
                  <div style="width: 24px; height: 24px; background-color: ${c}; border-radius: 4px; cursor: pointer; border: 1px solid #ffffff40;" @click=${() => onPaletteClick(c)}></div>
                `)}
              </div>
